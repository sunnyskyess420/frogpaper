...
SUBJECT_ALIASES = {
    ...
    "frog": "frog",
    "toad": "squat toad with rough warty skin, parotoid glands behind eyes, broader head, terrestrial stance",
    "bullfrog": "large bullfrog with wide mouth",
    "tree toad": "tree toad climbing on branch",
    "horned toad": "horned lizard with spiky scales",
    "fire-bellied toad": "fire-bellied toad with bright orange belly",
    "golden toad": "golden toad with shiny golden skin",
    "giant toad": "giant toad with massive body",
    "cute toad": "cute toad with friendly expression",
    "fantasy toad": "fantasy toad with magical aura",
    "cat": "cat",
    "cats": "cats",
    "kitten": "kitten",
    "kittens": "kittens",
    ...
    # Mythical creatures with strong identity constraints
    "banshee": "banshee spirit with flowing spectral form, ghostly female figure with tattered robes, pale translucent skin, hollow eyes, long disheveled hair, screaming mouth, ethereal wraith-like appearance",
    "dragon": "dragon with reptilian scales, massive wings, serpentine body, sharp claws, horned head, breathing fire",
    "phoenix": "phoenix with brilliant plumage, fiery wings, burning tail, rising from flames, immortal bird",
    "kraken": "kraken with massive tentacles, giant cephalopod, deep sea monster, beak, suckers",
    "unicorn": "unicorn with single spiraling horn, white horse body, magical mane, ethereal glow",
    "griffon": "griffon with eagle head and wings, lion body, talons, beak, majestic hybrid creature",
    "leviathan": "leviathan with massive serpentine body, sea dragon, aquatic monster, scales, fins",
    "chimera": "chimera with lion head, goat body, snake tail, multi-headed hybrid monster",
    "demon lord": "demon lord with horns, dark armor, menacing presence, infernal features, supernatural entity",
    "ancient god": "ancient god with divine radiance, powerful aura, supernatural form, deity",
    "void titan": "void titan with cosmic energy, dark matter form, eldritch features, otherworldly entity",
    "elder treant": "elder treant with ancient bark, massive tree form, gnarled branches, leafy canopy, sentient plant",
    "sea serpent": "sea serpent with elongated body, scales, fins, aquatic reptile, ocean monster",
    "angel of war": "angel of war with wings, armor, divine weapons, radiant form, celestial warrior",
    "forgotten deity": "forgotten deity with ancient robes, mystical symbols, divine presence, obscured face",
}
...
def get_lazy_keyword_expander():
    ...
    if _KEYWORD_EXPANDER is None and KEYWORD_EXPANSION_AVAILABLE:
        try:
            ...
            logger.warning("Could not initialize keyword expander: %s", e)
...
# Subject-specific negative constraints to prevent drift and substitutions
_SUBJECT_NEGATIVES = {
    "banshee": "owl, bird, eagle, hawk, raven, crow, falcon, any bird species, beak, feathers, wings, talons, animal, creature, beast",
    "dragon": "lizard, snake, crocodile, alligator, dinosaur, reptile, iguana, komodo, monitor lizard",
    "phoenix": "eagle, hawk, falcon, peacock, regular bird, parrot, flamingo, any non-phoenix bird",
    "kraken": "octopus, squid, jellyfish, fish, whale, shark, dolphin, sea creature, marine animal",
    "unicorn": "horse, pony, donkey, mule, zebra, regular equine, hornless horse",
    "griffon": "eagle, lion, regular bird, regular cat, panther, tiger, leopard",
    "leviathan": "whale, shark, fish, dolphin, sea serpent, regular sea creature",
    "chimera": "lion, goat, snake, regular animal, hybrid creature without three heads",
    "demon lord": "human, mortal, regular person, angel, divine being, saint",
    "ancient god": "human, mortal, regular person, demon, devil, evil entity",
    "void titan": "human, mortal, regular person, alien, robot, machine",
    "elder treant": "tree, plant, bush, shrub, regular vegetation without face",
    "sea serpent": "snake, eel, fish, shark, whale, regular aquatic reptile",
    "angel of war": "human, mortal, regular person, demon, devil, fallen angel",
    "forgotten deity": "human, mortal, regular person, statue, sculpture, inanimate object",
}


def get_subject_negatives(subject: str) -> str:
    """Get subject-specific negative constraints to prevent drift."""
    subject_lower = subject.lower().strip()
    for key, negatives in _SUBJECT_NEGATIVES.items():
...
# ═══════════════════════════════════════════════════════════════════════════
#  SUBJECT → SETTING BIAS
#  Maps underwater/aquatic subjects to compatible settings so they don't
#  end up floating in forests or perched on clifftops.
#  Each entry lists settings that work well with that subject type.
# ═══════════════════════════════════════════════════════════════════════════
_SUBJECT_SETTING_BIAS = {
    "underwater": [
...
def get_biased_setting(subject: str, setting: str, explicit_setting: bool) -> str:
    """If the subject requires a specific type of setting and no explicit
    setting was chosen by the user, pick one from the biased pool.

    Returns the (possibly overridden) setting string.
    """
    if explicit_setting and setting:
        return setting  # User explicitly chose a setting — respect it

    subject_lower = subject.lower()

    # Check each bias category
    for trigger_keywords, biased_settings in _SUBJECT_SETTING_BIAS.items():
        if trigger_keywords == "underwater":
            # Underwater triggers: kraken, leviathan, sea serpent, shark,
            # octopus, jellyfish, fish, whale, dolphin, submarine, etc.
            underwater_tokens = [
                "kraken", "leviathan", "sea serpent", "shark", "whale",
                "dolphin", "octopus", "squid", "jellyfish", "seahorse",
                "fish", "coral", "turtle", "crab", "lobster", "eel",
                "manatee", "ray", "narwhal", "pufferfish", "angelfish",
                "clownfish", "starfish", "submarine",
            ]
            if any(token in subject_lower for token in underwater_tokens):
                return random.choice(biased_settings)

        elif trigger_keywords == "flying":
            flying_tokens = [
                "eagle", "hawk", "falcon", "raven", "crow", "owl",
                "dragon", "phoenix", "griffon", "pterodactyl",
            ]
            if any(token in subject_lower for token in flying_tokens):
                return random.choice(biased_settings)
...
def get_theme_scene_mapping(subject: str) -> dict:
    ...
    # Underwater creature themes — add aquatic scene anchoring
    underwater_creatures = [
        "kraken", "leviathan", "sea serpent", "shark", "whale", "dolphin",
        "octopus", "squid", "jellyfish", "seahorse", "turtle", "crab",
        "lobster", "eel", "manatee", "ray", "narwhal", "submarine",
    ]
    if any(creature in subject_lower for creature in underwater_creatures):
        return {
            "focus": "creature",
            "scene_elements": [
                "underwater setting", "deep ocean", "aquatic atmosphere",
                "caustic light", "bubbles", "water current", "bioluminescent glow",
            ],
            "avoid": ["sky", "cloud", "mountain", "forest", "land", "ground",
                       "grass", "tree", "building interior", "studio", "table"]
        }
...
def build_sentence(subject: str, style: str, mood: str, color: str, lighting: str, atmosphere: str, tech: str, scenic_mode: bool, setting: str = "", subject_lock: bool = False, explicit_setting: bool = False) -> tuple[str, str]:
    """Build prompt sentence and return (sentence, subject_negatives)."""
    # Get theme-specific mapping
    theme_mapping = get_theme_scene_mapping(subject)
    
    # Get subject-specific negative constraints
    subject_negatives = get_subject_negatives(subject)
    ...
    # UNLESS the setting is explicitly provided by the user - in that case, keep it
    subject_lower = subject.lower().strip()
    if any(env in subject_lower for env in _ENV_SUBJECT_TOKENS) and not explicit_setting:
        scenic_mode = True
    ...
    # Filter inputs - but don't filter explicit user settings
    atmosphere = atmosphere if filter_element(atmosphere) else ""
    tech = tech if filter_element(tech) else ""
    # Don't filter setting if it was explicitly provided by the user
    ...
    if scenic_mode and not any(x in subject for x in ["cat", "frog", "doll"]):
        if subject_lock:
            parts = [f"{_article(subject)} {subject}"]
        else:
    ...
    if theme_mapping["focus"] == "character":
        ...
        scene_layers = []
        if subject_lock:
            scene_layers.append(f"{_article(subject)} {subject} depicted in a {style} art style")
        else:
        ...
        if env_details:
            if setting and env_details[0] == setting:
                # Add grounding for character in setting
                grounding_phrases = ["standing on", "sitting on", "perched on", "resting on", "positioned on"]
                scene_layers.append(f"{random.choice(grounding_phrases)} a {setting}")
        ...
        scene_layers = []
        if subject_lock:
            scene_layers.append(f"{_article(subject)} {subject} in {style} style")
        else:
        ...
        if setting:
            # Add grounding for subject in setting
            grounding_phrases = ["standing on", "sitting on", "perched on", "resting on", "positioned on"]
            scene_layers.append(f"{random.choice(grounding_phrases)} a {setting}")
        elif has_atmosphere:
            # User selected explicit atmosphere - use it as the primary environment
            grounding_phrases = ["standing in", "sitting in", "perched in", "resting in", "positioned in"]
            scene_layers.append(f"{random.choice(grounding_phrases)} {atmosphere}")
        else:
...
def generate_themes(count: int = 5, user_keywords=None, subject_lock: bool = True, custom_subject: str = "",
    ...
    logger.debug("generate_themes called with:")
    logger.debug("  explicit_subject=%r", explicit_subject)
    logger.debug("  explicit_style=%r", explicit_style)
    logger.debug("  explicit_lighting=%r", explicit_lighting)
    logger.debug("  explicit_setting=%r", explicit_setting)
    logger.debug("  explicit_atmosphere=%r", explicit_atmosphere)
    logger.debug("  explicit_mood=%r", explicit_mood)
    logger.debug("  explicit_color=%r", explicit_color)
    logger.debug("  subject_lock=%s", subject_lock)
    logger.debug("  custom_subject=%r", custom_subject)
    ...
    for i in range(count):
        ...
        if explicit_atmosphere:
            ...
            if atmosphere_pool:
                atmosphere = random.choice(atmosphere_pool)
            else:
                atmosphere = choose_from_alias_or_pool(
                    user_buckets.get("atmosphere", []), ATMOSPHERE_ALIASES, [], "soft atmospheric depth"
                )
        
        # Setting: use explicit user selection if provided
        setting = explicit_setting if explicit_setting else ""

        # Apply subject→setting bias so aquatic subjects land in water,
        # flying subjects land in the sky, etc.  Only when user didn't
        # explicitly pick a setting.
        if not explicit_setting:
            setting = get_biased_setting(subject, setting, explicit_setting=False)
...
def print_themes(themes: list[dict]) -> None:
    logger.debug("=" * 60)
    logger.debug("GENERATED THEMES")
    logger.debug("=" * 60)
    for t in themes:
        logger.debug("Theme #%s", t['theme_id'])
        logger.debug("  %s", t['sentence'])
        logger.debug("  Composition: %s", t['components']['composition'])
    logger.debug("=" * 60)
