...
USER_THESAURUS_FILE = BASE_DIR / "user_thesaurus.json"
LOGS_DIR = BASE_DIR / "logs"
LOGS_DIR.mkdir(exist_ok=True)
EXPANSION_LOG = LOGS_DIR / "keyword_expansion.json"
...
class KeywordExpander:
    ...
    def warmup(self):
        """Pre-warm NLTK and sentence-transformers on app startup (reduces cold-start latency)."""
        ...
        if self._check_nltk():
            try:
                ...
                print(f"Warning: NLTK warmup failed: {e}")
        ...
        if self._check_sentence_transformers():
            try:
                ...
                print(f"Warning: Sentence-transformers warmup failed: {e}")
    ...
    def _load_user_thesaurus(self):
        """Load user-defined thesaurus mappings."""
        try:
            if USER_THESAURUS_FILE.exists():
                with open(USER_THESAURUS_FILE, 'r', encoding='utf-8') as f:
                    self.user_thesaurus = json.load(f)
            else:
                self.user_thesaurus = {}
        except Exception as e:
            print(f"Error loading user thesaurus: {e}")
            self.user_thesaurus = {}
    ...
    def find_semantic_similarities(self, word: str, top_k: int = 3) -> List[Tuple[str, float]]:
        ...
        try:
            ...
            print(f"Error finding semantic similarities: {e}")
            return []
    ...
    def _save_user_thesaurus(self):
        """Save user thesaurus to file."""
        try:
            with open(USER_THESAURUS_FILE, 'w', encoding='utf-8') as f:
                json.dump(self.user_thesaurus, f, indent=2)
        except Exception as e:
            print(f"Error saving user thesaurus: {e}")
    ...
    def _save_expansion_log(self):
        """Save expansion history to log file."""
        try:
            with open(EXPANSION_LOG, 'w', encoding='utf-8') as f:
                json.dump(self.expansion_history[-100:], f, indent=2)  # Keep last 100 entries
        except Exception as e:
            print(f"Error saving expansion log: {e}")
...
def warmup_keyword_expander():
    ...
    try:
        ...
        print(f"Warning: Could not warmup keyword expander: {e}")
